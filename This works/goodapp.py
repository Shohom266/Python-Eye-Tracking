...
def jog_axis_velocity(axis_index: int, direction: int, axis_count: int = 6, magnitude: float = 0.2):
    vel = [0.0] * axis_count
    vel[axis_index] = magnitude if direction >= 0 else -magnitude
    return vel
...
def safe_joint_jog(joint, direction, hold_cycles=200, cycle_sleep=0.01):
    r = require_robot()
    try:
        if not r.is_robot_in_automatic_mode():
            raise RuntimeError("Robot is not in Automatic mode on the pendant.")
        ...
        speed_scale=max(0.2, min(1.0, current_velocity / 100.0))

        jog_velocity = [0.0]* 6
        jog_velocity[joint-1]= speed_scale if direction>0 else -speed_scale

        before= list(r.get_current_joint_angles())

        r.turn_on_jog(jog_velocity=jog_velocity, jog_type="Joint")
        try:
            for _ in range(max(20, int(hold_cycles))):
...
def api_command(cmd: Command):
    global current_step_mm, current_velocity, current_jog_mode, robot_started
